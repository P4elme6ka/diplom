            ...
            foreach ($current_acceptance_provider->getModels() as $acceptance_group) { ?>
                <div class="card mb-3">
                    <div class="card-body">
                        ...
                        <a href="<?= Url::to(['acceptanceclass/view', 'id' => $acceptance_group['id']]) ?>"
                           class="card-link">Посмотреть рейтинги</a>
                        <?php if (!Yii::$app->user->isGuest) { ?>
                            <a href="<?= Url::to(['createacc']) ?>" class="card-link">Подать заявление</a>
                        <?php } ?>
                        <p class="card-text"><small class="text-muted">Последние
                                обновление <?= $acceptance_group["time"] ?></small></p>
                    </div>
                </div>
            <? }
        } ?>
